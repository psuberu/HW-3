...
* We also consider all 222,540 songs played in the Reed College pool hall from
Nov 30, 2003 to Jan 22, 2009 (included in `HW-3` folder). Be sure to do a quick
EDA on the data before delving into the questions.
...
user $i$'s sex and save this in a variable `predicted_sex`.  i.e. for user $i$

* If $\widehat{p}_i > p^*=0.5$, set `predicted_sex = 1` i.e. female
* If $\widehat{p}_i < p^*=0.5$, set `predicted_sex = 0` i.e. male
...
decision threshold $p^*=0.5$ should we use?
...
* On the x-axis present actual dates (not numbers like Week 456, which doesn't 
mean anything to most people). Having the `x` aesthetic reflect actual dates
...
Using the jukebox data, what are the top 10 artists played
during the "graveyard shift" during the academic year. Define
...
deliverable insight to a foreign currency exchanger.
